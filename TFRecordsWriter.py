...
class TFRecordGenerator:
    ...
    def _convert_image(self, img_path, label):
        ...
        if image_data.mode == 'RGB':
            print('RBG here')
        image_data_resized = image_data.resize((28,28))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='convert images to TFrecord file.')
    parser.add_argument('-i', '--input-folder', metavar='', required=True, help='the images folder')

    parser.add_argument('-o', '--output-file', metavar='', default='dataset.tfr' ,help='the output TFRecord name')

    args = parser.parse_args()
    
    tfrecord_gen = TFRecordGenerator()

    tfrecord_gen.convert_image_folder(args.input_folder, args.output_file)
